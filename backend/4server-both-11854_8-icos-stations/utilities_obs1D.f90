...
subroutine setdim2(n,m,k)
  ...
  print*, 'ng = ', ng
  ! get dimid nobs
  rc = nf90_inq_dimid(ncid, "nobs", dimid); call rc_handler(rc, .true.)
  ! get nobs
  rc = nf90_inquire_dimension(ncid, dimid, len=nobs)
  rc = nf90_close(ncid)
  print*, 'nobs = ', nobs
...
subroutine setval2 (n, m, k, x0, sx0, xp, y, sy, yinit, oj, tj, twin)
  ...
  logical, intent(out) :: twin ! setup for twin experiment
! local
  integer ( kind = 4 ) :: i,j,l,ista,iobs
  real ( kind = 8 ) :: pert=1.1_8 ! perturbation for identical twin
  real ( kind = 8 ) :: minsx0 ! floor for prior uncertainty
  integer ( kind = 4 ) :: rc, ncid, varid, no_fill
  real ( kind = 8 ) :: fill_value ! 
  real ( kind = 8 ) :: ojin(ng, nobs) ! input obs Jacobian [ppb/(kgCH4/cell)]
  real ( kind = 8 ) :: yin(nobs) ! input obs [ppb]
  real ( kind = 8 ) :: tjin(ng, ntgt) ! input target Jacobian [kgCH4/(kgCH4/cell)]
  logical :: mask(nobs)     ! to be set false for fill values
  logical :: complete(nobs) ! to be set false if any fill value
  ...
  !-- MVO-NOTE::in the 1D observation vector we should not expect missing values
  mask=.true.
  where (yin.ne.yin)
     mask=.false.
  endwhere
! count sites without missing values
  ...
  do iobs=1,nobs
     write(11,'(i10,1x,31l10)') iobs,mask(iobs)
     complete(iobs) = mask(iobs)
  enddo
  write(11,'(a,i10)') '# of sites with complete obs recored = ', count(complete)
  call flush(11)
  print*, '# of valid obs = ', count(mask), ' from ', nobs
  write(10,'(a10,1x,31i10)') 'site#', (i,i=1,31)
  do iobs=1,nobs
     write(10,'(i10,1x,31f10.1)') iobs,yin(iobs)
     write(10,'(i10,1x,31l10)') iobs,mask(iobs)
  enddo
  ! FILL_VALUE=99.
...
!  print*, 'ncid, varid = ', ncid, varid
  ...
  rc = nf90_close(ncid)
  oj = transpose(ojin) !transpose(reshape(ojin,(/ng,nobs/)))
...
subroutine wemission(n,x)
  ...
  ! local
  integer ( kind = 4 ) :: rc, ncid, varid, ngdimid
  ! open file
  rc = nf90_open(fepost,nf90_write, ncid); call rc_handler(rc, .true.)
  ! get dimids
  rc = nf90_inq_dimid(ncid, "ng", ngdimid); call rc_handler(rc, .true.)
  rc = nf90_enddef(ncid)
  rc = nf90_def_var(ncid, "emission", nf90_real8, (/ ngdimid /), varid)
  call rc_handler(rc, .true.)
  rc = nf90_put_var(ncid, varid, x/kg2mt) ! back to kgCH4/cell
  ! set attributes
  rc = nf90_put_att(ncid, varid, "long_name", "ch4_emission")
  rc = nf90_put_att(ncid, varid, "units", "kgCH4/cell")
  rc = nf90_close(ncid)
...
subroutine wconc(m,y)
  ...
  ! local
  integer ( kind = 4 ) :: rc, ncid, yvarid, nobsdimid
  ! open file
  rc = nf90_open(fc,nf90_write, ncid); call rc_handler(rc, .true.)
  rc = nf90_inq_dimid(ncid, "nobs", nobsdimid); call rc_handler(rc, .true.)
  rc = nf90_def_var(ncid, "conc", nf90_real8, (/ nobsdimid /), yvarid)
  ! set attributes
  rc = nf90_put_att(ncid, yvarid, "long_name", "simulated CH4 concentration")
  rc = nf90_put_att(ncid, yvarid, "units", "ppb")
  print*, 'conc defined'; call flush(6)
  rc = nf90_enddef(ncid)
  rc = nf90_put_var(ncid, yvarid, y)
  rc = nf90_close(ncid)
...
subroutine wconcpost(m,y0,y,yobs)
  ...
  ! local
  integer ( kind = 4 ) :: rc, ncid, varid, nobsdimid, yvarid, y0varid
  print*, 'in wconc: '
  ...
  rc = nf90_inq_dimid(ncid, "nobs", nobsdimid); call rc_handler(rc, .true.)
  ! define variables
  rc = nf90_def_var(ncid, "cpost", nf90_real8, (/ nobsdimid /), yvarid)
  call rc_handler(rc, .true.)
  rc = nf90_def_var(ncid, "cprior", nf90_real8, (/ nobsdimid /), y0varid)
  call rc_handler(rc, .true.) 
  print*, 'conc defined'; call flush(6)
  rc = nf90_enddef(ncid)
  ! set attributes
  ...
  rc = nf90_put_var(ncid, y0varid, y0)
  call rc_handler(rc, .true.) !-- check writing data went properly 
  ! close file
  rc = nf90_close(ncid)
