...
class conGrad_MC(conGrad):
    """
    This is a class to implement Frédéric Chevallier's approach to calculating posterior uncertainties.
    We start 4DVAR simulations with a bunch of different priors, all consistent with the prior flux
    PDF. Then we run a 4DVAR on each of them, and once they converge, we calculate the posterior PDF
    from the spread of the solutions. However, the prior PDF is always under-sampled due to the finite
    ensemble size. So what we do is calculate the uncertainty reduction as (posterior spread)/(prior spread),
    then multiply that by the prior uncertainty to get the posterior uncertainty.

    The way our jobs are set up, the optimizer doens't even have to know that it's a member of an ensemble.
    All we need is to initialize the optimizer away from the prior flux, in accordance with the prior PDF.
    """
    def Var4Dsetup_fwd(self):
        # first forward run during Var4Dsetup, to be done with a perturbed state vector
        self.state_vec = random.standard_normal(self.tm5.state_preco.shape)
        # Store this prior state vector
        self.tm5.state_prior_preco = zeros_like(self.state_vec)
        self.tm5.state_prior_preco[:] = self.state_vec
    ...
    def EvalFunc_mid(self):
        """
        This is identical to EvalFunc_mid of the class Optimizer, except that there is a facility to perturb
        the measurements according to their error statistics. This is achieved by creating the perturbations
        right after StoreModelReprErrors executed during the first iteration, and then during subsequent iterations
        right after RestoreModelReprErrors. The Optimizer class (or, in this case, conGrad_MC) should know nothing
        of the perturbations. The creation of the perturbations should be handled by RunTM5, and their application
        should be done by the point and satellite classes.
        """
        if self.converged:
            # If this run was after convergence, save stations.nc4 to stations_apos.nc4, and tccon.nc4 to tccon_apos.nc4
            if self.tm5.rcf.get('output.station.timeseries', 'bool'):
        ...
        if self.rcf.get('adjoint.input.satellite', 'bool') or self.rcf.get('adjoint.input.point', 'bool'):
            if self.first_run:
                self.tm5.StoreModelReprErrors()
                # Begin modification for conGrad_MC
                self.tm5.CreateMeasurementPerturbations()
                # End modification for conGrad_MC
            else:
                self.tm5.RestoreModelReprErrors()
    ...
    def FinishUp(self, optim_output_file, use_eigen_file=False):
        """
        The only difference between the FinishUp method in the base class and this one is that emission_model_prior
        corresponds to the perturbed initial state vector, not x_c=0. Also, we don't really need to convert the eigenvectors
        to model space, because they will not be used anyway.
        """
        if os.path.exists(optim_output_file):
            os.remove(optim_output_file)
        # Fill self.tm5 with zoom region info
        ...
        # Begin modification for conGrad_MC
        with Dataset(self.comm_file, 'r') as fid:
            state_vec = fid.variables['x_c'][:,0]
        state_model = self.tm5.preco.xc_to_x(state_vec, self.tm5.state_prior_model)
        emission_model_prior, iniconc_model_prior, param_model_prior = self.tm5.preco.state2struct(state_model)
        # End modification for conGrad_MC
        sys.stdout.write('done!\n') ; sys.stdout.flush()

        sys.stdout.write('Optimizer.FinishUp :: calculating apri errors... ') ; sys.stdout.flush()
        apri_std_emis, apri_std_iniconc, apri_std_param = self.tm5.preco.state2struct(self.tm5.preco.G_state, \
            replace_field=self.tm5.preco.emis_dapri)
        sys.stdout.write('done!\n') ; sys.stdout.flush()
        ...
        del emission_model, emission_model_prior, iniconc_model, iniconc_model_prior, param_model, param_model_prior, state_vec, state_model

class m1qn3(Optimizer):
    """
    The older m1qn3 class adapted for the multitracer 4DVAR by Sudhanshu Pandey and Maarten Krol. Efforts ongoing to adapt
    it to the more updated multitracer code of Sourish Basu.

    The exit codes for m1qn3 are different from those for congrad. The variable m1qn3_finished, written as an attribute
    of the state file, has the following values:

    3: finished because gradient norm converged
    """
    def __init__(self, tm5Obj, resume=False, cleanup=False):
        self.output_dir = tm5Obj.output_dir
        self.tm5 = tm5Obj
    ...
    def Var4Dsetup_adj(self):
        ...
        fid.createDimension('dim_x', 0)
        fid.createDimension('dim_g', 0)
        fid.createDimension('dim_f', 0)
        fid.iter_max = self.max_iter
        fid.iter_convergence = self.fixed_iterations
        fid.preduc = 1./self.grad_norm_reduc_factor
        fid.m1qn3_finished = 0
        var = fid.createVariable('J_tot', self.state_vec.dtype, ( 'dim_f'))
        var[0] = self.cost_fn
        var = fid.createVariable('x_c', self.state_vec.dtype, ('n_state', 'dim_x'))
        var[:,0] = self.state_vec
        var = fid.createVariable('g_c', self.state_vec.dtype, ('n_state', 'dim_g'))
    ...
    def Var4Dstep_check(self):
        ...
        self.checkStateFile(self.comm_file)
        sbp.check_call(self.OptimCommand)
        # after the call, there are n gradients and n+1 state vectors in the file
        # the (n+1)th state vector is where the optimizer is asking for the gradient now
        with Dataset(self.comm_file, 'r') as fid:
            self.true_iter = len(fid.dimensions['dim_x']) - 1
            self.optim_finished = fid.m1qn3_finished
    ...
    def read_xc(self):
        with Dataset(self.comm_file, 'r') as fid:
            ...
            # Has the last run also been done?
            len_g = len(fid.dimensions['dim_g'])
            self.last_run_done = (len_x == len_g)

    def write_gc(self):
        with Dataset(self.comm_file, 'a') as fid:
            len_g = len(fid.dimensions['dim_g'])
            fid.variables['g_c'][:,len_g] = self.gradient_preco
            len_f = len(fid.dimensions['dim_f'])
            fid.variables['J_tot'][len_f] = self.cost_fn
            fid.J_tot = self.cost_fn

    def Var4Dstep(self):
        self.Var4Dstep_check()
        if self.optim_finished == 0: # not yet converged and max_iter not reached
            self.read_xc()
            self.OptimFunc(self.state_vec)
            self.write_gc()
        elif self.optim_finished == 1: # not converged but max_iter reached
            self.finished = True
        elif self.optim_finished == 2: # converged
            self.converged = True
            self.finished = True
            self.read_xc()
            if not self.last_run_done:
                sys.stderr.write("Optimization complete, running forward with final state vector\n")
                self.OptimFunc(self.state_vec)
                self.write_gc()
            else:
                print 'Last run need not be done, simply reading the current state vector for finishing up'
    ...
    def Var4Dsetup(self):
        ...
        if self.Restart:
            fid = Dataset(self.comm_file, 'r')
            m1qn3_finished = fid.m1qn3_finished
            fid.close()
            self.restoreFile(m1qn3_finished)
        else:
            # Should run self.OptimFunc(self.tm5.state_preco), but split it up into fwd and adj runs
            self.Var4Dsetup_fwd()
            self.Var4Dsetup_adj()
    ...
    def Var4Ddone(self):
        if self.converged:
            ...
            optim_output_file = self.optim_state_file_name()
            print 'Calling FinishUp with ', optim_output_file
            self.FinishUp(optim_output_file)
    ...
    def restoreFile(self, mode):
        ...
        if mode == 0:
            ...
            # we need to knock out that last state vector
            randString = ''.join(choice(string.ascii_letters + string.digits) for x in range(10))
            randFile = os.path.join(os.path.dirname(self.OptimCommand[1]), randString+'.nc4')
            inFid = Dataset(self.comm_file, 'r')
            outFid = Dataset(randFile, 'w')
            # copy the attributes
            outFid.m1qn3_finished = 0
            outFid.iter_max = inFid.iter_max
            outFid.preduc = inFid.preduc
            outFid.iter_convergence = self.fixed_iterations
            # copy the dimensions
            ...
            outFid.createDimension('dim_x', 0)
            outFid.createDimension('dim_g', 0)
            outFid.createDimension('dim_f', 0)
            # copy the variables
            dim_g = len(inFid.dimensions['dim_g'])
            v = outFid.createVariable('J_tot',  inFid.variables['J_tot'].dtype, ( 'dim_f'))
            v[:] = inFid.variables['J_tot'][:dim_g]
            v = outFid.createVariable('x_c', inFid.variables['x_c'].dtype, ('n_state','dim_x'))
            v[:] = inFid.variables['x_c'][:,:dim_g]
            v = outFid.createVariable('g_c', inFid.variables['g_c'].dtype, ('n_state','dim_g'))
            v[:] = inFid.variables['g_c'][:,:dim_g]
            ...
            v = outFid.createVariable('g_c', inFid.variables['g_c'].dtype, ('n_state','dim_g'))
            v[:] = inFid.variables['g_c'][:,:dim_g]
            v = outFid.createVariable('J_tot', inFid.variables['J_tot'].dtype, ('n_state','dim_f'))
            v[:] = inFid.variables['J_tot'][:dim_g]
            outFid.close()
            inFid.close()
            shutil.move(randFile, self.comm_file)
    ...
    def shaveOffIterations(self, num_iters_x, num_iters_g=None):
        ...
        # copy the variables
        v = outFid.createVariable('J_tot', inFid.variables['J_tot'].dtype, ('dim_f',))
        v[:] = inFid.variables['J_tot'][:num_iters_f]
        v = outFid.createVariable('x_c', inFid.variables['x_c'].dtype, ('n_state','dim_x'))
        v[:] = inFid.variables['x_c'][:,:num_iters_x]
        v = outFid.createVariable('g_c', inFid.variables['g_c'].dtype, ('n_state','dim_g'))
        v[:] = inFid.variables['g_c'][:,:num_iters_g]
