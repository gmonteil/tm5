...
from pyshell.tm5_utils import precon
...
class Precon(object):
    ...
    def read_apri_std(self, apri_file):
                            ...
                            if self.optim_type[tracer][cat] == 1:
                                ...
                                if enforce_minerror:
                                    ...
                                    if min_std_average_flag:
                                        self.emis_dapri[region][tracer][cat] = {'emission_data': err_mask * \
                                            maximum(grid_error, min_std * average(abs(emission)) * ones_like(emission))}
                                    else:
                                        self.emis_dapri[region][tracer][cat] = {'emission_data': err_mask * \
                                            maximum(grid_error, min_std * ones_like(grid_error))}
                                else:
                                    self.emis_dapri[region][tracer][cat] = {'emission_data': err_mask * grid_error}

                            elif self.optim_type[tracer][cat] == 2: # SB's transformation
                                self.emis_dapri[region][tracer][cat] = {'emission_data': \
                                    err_mask * grid_error}
                                ...
                                self.emis_dapri[region][tracer][cat] = {'emission_data': \
                                    err_mask * ones_like(grid_error)*error*0.01}

                            else: # optim_type is not 1, 2 or 3, but code below is identical to 3
                                self.emis_dapri[region][tracer][cat] = {'emission_data': \
                                    err_mask * ones_like(emission)*error*0.01}
        ...
        if self.Optim_dict['iniconc']:
            print 'Not implemented yet, quit program in read_apri_std, class Precon...'
            sys.exit(0)
        else:
            iniconc_dapri = None

        G = self.struct2state(self.emis_dapri,iniconc_dapri,parameters_dapri)
        return G
    ...
    def calc_gaussian_corr(self, corlen, n):
        A = zeros((n,n))
        P = zeros((n,n))
        D = zeros((n,n))
        lam = zeros(n)
        if corlen<1.e-20:
            ...
            dummy_X, dummy_Y = meshgrid(arange(n), arange(n))
            A = exp(-(abs(dummy_X-dummy_Y)/corlen)**2)
            P,D = self.matrix_square_root(A)
            #lam,P = linalg.eigh(A)
            ## Sort eigenvalues and switch columns of P1 accordingly.
    ...
    def struct2state(self,emission=None,iniconc=None,parameters=None):
        ...
        if iniconc is not None:
           print 'Iniconc optimization not implemented'
           sys.exit(3)
            ...
            for tracer in self.species:
                for c, cat_key in enumerate(self.Emission[tracer]['cat_list']):
                    ...
                    if os.path.isfile(vec2ll_file):
                        ...
                        print 'vec2ll file not yet created. Done in forward run => do this first'
                        sys.exit(2)
        ...
        if self.Optim_dict['emission']:
            ...
            for tracer in self.species:
                for c,cat_key in enumerate(self.Emission[tracer]['cat_list']):
                    ...
                    cat_blk = nt*n_hor
                    x += precon.xc_to_x(self.G_state,Temp_L,Hor_L,x_c,ipos,self.max_threads)
                    ipos += cat_blk
            t2 = datetime.now()
            sys.stderr.write("Emission preconditioned in %s\n"%str(t2-t1))

        if self.Optim_dict['iniconc']:
            print 'Iniconc preconditioning is not implemented yet in xc_to_x in class Precon...'
            sys.exit(0)
    ...
    def g_to_gc(self, g): # Pim: add stuff for adj_iniconc
        ...
        if self.Optim_dict['emission']:
            ...
            for tracer in self.species:
                for c,cat_key in enumerate(self.Emission[tracer]['cat_list']):
                    ...
                    cat_blk = nt*n_hor
                    g_c += precon.g_to_gc(self.G_state,Temp_Lt,Hor_Lt,g,ipos,self.max_threads)
                    ipos += cat_blk
        ...
        if self.Optim_dict['iniconc']:
            print 'Iniconc preconditioning is not implemented yet in g_to_gc in class Precon...'
            sys.exit(0)
        ...
        with open(file_name, 'wb') as fid:
            ...
            for k, v in self.__dict__.items():
                ...
                if isinstance(v, ndarray):
                    fname = os.path.join(state_dir, 'precon_%s.npy'%k)
                    save(fname, v)

                # No need to save the 'Emission' object, since it will be saved by RunTM5. Also no need to save 'tm5'.
                elif k in ['Emission', 'tm5']:
                    pass
