...
import pickle
import glob
from numpy import load, int32, linalg, zeros_like, nan_to_num, sqrt, inner, array, int16, float64
import sys
from netCDF4 import Dataset
from random import choice
import string
import shutil
from pyshell import rc
import subprocess
...
try :
    ...
    logger.warning("Warning: postprocess module not loaded\n")


class Congrad:
    def __init__(self, model, resume = False, cleanup = False):
        self.tm5 = model
        self.output_dir = self.tm5.output_dir

        if resume:
            self.load(cleanup=cleanup)
            logger.debug("Loading previously saved Optimizer data")
        else :
            ...
            if self.tm5.Optim_dict['emission']:
                self.EmisFile = self.tm5.emission_file_name
            else:
                self.EmisFile = None
            self.ParamFile = os.path.join(self.output_dir, self.rcf.get('parameter.scratch.filename')) if self.tm5.Optim_dict['parameters'] else None
            self.IniFile = 'NotDoneYet.txt' if self.tm5.Optim_dict['iniconc'] else None
            ...
            self.true_iter = 0
            self.fixed_iterations = self.tm5.dconf.optim.get('n_iter', 1000)
    ...
    def Var4Dstep(self):
        ...
        if self.congrad_finished == 0: # not yet converged and max_iter not reached
            ...
            if not self.last_run_done:
                ...
                logger.info('Last run need not be done, simply reading the current state vector for finishing up')
            self.read_eigsys()
    ...
    def Var4Dstep_check(self):
        ...
        subprocess.check_call(self.OptimCommand)
    ...
    def Var4Ddone(self):
        ...
        if self.converged == 1:
            ...
            logger.info('Calling FinishUp with ', optim_output_file)
            self.FinishUp(optim_output_file, True)
            logger.info("VAR4D :: Optimization finished successfully\n")
        else:
            if self.finished:
                logger.info("VAR4D :: Optimization failed because max_iter was reached\n")
            else:
                logger.info("VAR4D :: Optimization failed for unknown reason, please investigate\n")
            ...
            for reg_num, region in enumerate(self.tm5.region_names):
                ...
                for tracer in self.species:
                    ...
                    for category in self.tm5.Emission[region][tracer]['categories']:
                        ...
                        if self.converged_eigvals is not None:
                            logger.info("Writing eigenvectors and eigenvalues for region %s tracer %s category %s"%(region,tracer,category))
    ...
    def restoreFile(self, mode):
        logger.info("restoreFile called with mode %i" % mode)
        ...
        if mode == 0:
            ...
            outFid.close()
            inFid.close()
            logger.info("Both x_c and g_c reduced to %i vectors" % dim_g)
    ...
    def load(self, cleanup=True):
        ...
        t2 = datetime.now()
        logger.info("Optimizer state loaded in %s"%str(t2-t1))