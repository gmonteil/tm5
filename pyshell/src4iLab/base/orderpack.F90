...
! ORDERPACK 2.0 -- Unconditional, Unique, and Partial Ranking,
! Sorting, and Permutation Downloadable Fortran 90 source code
! Author: Michel Olagnon
! Go to http://www.fortran-2000.com/rank/ for full documentation
! Adapted to TM5 by Sourish Basu (2010)
...
public :: multiplicity
private :: D_multiplicity, R_multiplicity, I_multiplicity

public :: binarySearch, searchSorted
private :: D_searchSorted, R_searchSorted, I_searchSorted

Integer, Parameter :: ksp = selected_real_kind(2), kdp = selected_real_kind(15) ! single precision and double precision
...
interface multiplicity
    module procedure D_multiplicity, R_multiplicity, I_multiplicity
end interface multiplicity
...
pure function D_searchSorted(in_array, value) result(idx)
...
pure function R_searchSorted(in_array, value) result(idx)
    ! __________________________________________________________
...
pure function I_searchSorted(in_array, value) result(idx)
    ! __________________________________________________________
...
subroutine D_multiplicity(in_array, out_uniq_elems, out_mulcnt, sorted)
    ! __________________________________________________________
    !   D_multiplicity = return the unique elements in an array and their
    !   multiplicity. The unique elements are always returned in out_uniq_elems,
    !   and their corresponding multiplicities in out_mulcnt.
    !   'sorted' is an optional argument that specifies whether the input
    !   array is already sorted or not. The default is to assume unsorted, in
    !   which case mrgrnk is called to sort it first. Setting 'sorted' to .true.
    !   avoids that extra procedure call.
    ! __________________________________________________________
    ! __________________________________________________________
    real (kind=kdp), dimension(:), intent(in)               :: in_array
    real (kind=kdp), dimension(:), allocatable, intent(out) :: out_uniq_elems
    integer, dimension(:), allocatable, intent(out)         :: out_mulcnt
    logical, optional, intent(in)                           :: sorted
    ! __________________________________________________________
    real (kind=kdp), dimension(:), allocatable              :: scratch_space
    integer, dimension(:), allocatable                      :: imult, rank_array
    integer                                                 :: nuni, i
    logical                                                 :: lets_sort
    ! __________________________________________________________
    allocate(imult(size(in_array)))
    allocate(rank_array(size(in_array)))
    allocate(scratch_space(size(in_array)))
    if (present(sorted)) then
        lets_sort = .not. sorted
    else
        lets_sort = .true.
    end if
    if (lets_sort) then
        call mrgrnk(in_array, imult)
        do i=1,size(in_array)
            scratch_space(i) = in_array(imult(i))
        end do
    else
        scratch_space(:) = in_array(:)
    end if
    ! scratch_space is the sorted input array
    call mulcnt(scratch_space, imult)
    call unirnk(scratch_space, rank_array, nuni)
    allocate(out_uniq_elems(nuni))
    allocate(out_mulcnt(nuni))
    do i=1,nuni
        out_uniq_elems(i) = scratch_space(rank_array(i))
        out_mulcnt(i) = imult(rank_array(i))
    end do
    deallocate(imult)
    deallocate(rank_array)
    deallocate(scratch_space)
end subroutine D_multiplicity

subroutine R_multiplicity(in_array, out_uniq_elems, out_mulcnt, sorted)
    ! __________________________________________________________
    !   R_multiplicity = return the unique elements in an array and their
    !   multiplicity. The unique elements are always returned in out_uniq_elems,
    !   and their corresponding multiplicities in out_mulcnt.
    !   'sorted' is an optional argument that specifies whether the input
    !   array is already sorted or not. The default is to assume unsorted, in
    ...
    ! __________________________________________________________
    real (kind=ksp), dimension(:), intent(in)               :: in_array
    real (kind=ksp), dimension(:), allocatable, intent(out) :: out_uniq_elems
    integer, dimension(:), allocatable, intent(out)         :: out_mulcnt
    logical, optional, intent(in)                           :: sorted
    ! __________________________________________________________
    real (kind=ksp), dimension(:), allocatable              :: scratch_space
    integer, dimension(:), allocatable                      :: imult, rank_array
    integer                                                 :: nuni, i
    logical                                                 :: lets_sort
    ! __________________________________________________________
    ...
    deallocate(rank_array)
    deallocate(scratch_space)
end subroutine R_multiplicity

subroutine I_multiplicity(in_array, out_uniq_elems, out_mulcnt, sorted)
    ! __________________________________________________________
    !   I_multiplicity = return the unique elements in an array and their
    !   multiplicity. The unique elements are always returned in out_uniq_elems,
    !   and their corresponding multiplicities in out_mulcnt.
    !   'sorted' is an optional argument that specifies whether the input
    !   array is already sorted or not. The default is to assume unsorted, in
    ...
    ! __________________________________________________________
    integer, dimension(:), intent(in)               :: in_array
    integer, dimension(:), allocatable, intent(out) :: out_uniq_elems
    integer, dimension(:), allocatable, intent(out) :: out_mulcnt
    logical, optional, intent(in)                   :: sorted
    ! __________________________________________________________
    integer, dimension(:), allocatable              :: scratch_space
    integer, dimension(:), allocatable              :: imult, rank_array
    integer                                         :: nuni, i
    logical                                         :: lets_sort
    ! __________________________________________________________
...
! _________________________________________________________
      ...
      Integer, Dimension (:), Intent (Out) :: IGOEST
! __________________________________________________________
      Real (kind=ksp)   :: XTST, XDONA, XDONB
!
! __________________________________________________________
      Integer, Dimension (SIZE(IGOEST)) :: JWRKT, IRNGT
      Integer :: LMTNA, LMTNC, IRNG, IRNG1, IRNG2, NUNI
...
! _________________________________________________________________
! _________________________________________________________
      Real (kind=ksp), Dimension (:), Intent (InOut) :: XDONT
      Real, Intent (In) :: PCLS
! __________________________________________________________
!
      Real, Dimension (Size(XDONT)) :: XINDT
...
! _________________________________________________________
      Real (kind=ksp), Dimension (:), Intent (In) :: XDONT
      Real (kind=ksp) :: FNDNTH
      Integer, Intent (In) :: NORD
! __________________________________________________________
      Real (kind=ksp), Dimension (NORD) :: XWRKT
      Real (kind=ksp) :: XWRK, XWRK1
!
!
      Integer :: ICRS, IDCR, ILOW, NDON
!
...
!  average for random uniformly distributed values.
! __________________________________________________________
      Real (kind=ksp), Dimension (:), Intent (In) :: XDATT
      Integer, Dimension (:), Intent (In) :: IDATT
      Integer, Intent (Out) :: ires_med
! __________________________________________________________
!
      Real (kind=ksp), Parameter :: XHUGE = HUGE (XDATT)
      Real (kind=ksp) :: XWRK, XWRK1, XMED7, XMAX, XMIN
!
      Integer, Dimension (7*(((Size (IDATT)+6)/7+6)/7)) :: ISTRT, IENDT, IMEDT
      Integer, Dimension (7*((Size(IDATT)+6)/7)) :: IWRKT
      Integer :: NTRI, NMED, NORD, NEQU, NLEQ, IMED, IDON, IDON1
...
! _________________________________________________________
      ...
      Integer, Intent (In) :: NORD
! __________________________________________________________
      Real (kind=ksp) :: XPIV, XPIV0, XWRK, XWRK1, XMIN, XMAX
!
      Integer, Dimension (NORD) :: IRNGT
      Integer, Dimension (SIZE(XDONT)) :: ILOWT, IHIGT
      Integer :: NDON, JHIG, JLOW, IHIG, IWRK, IWRK1, IWRK2, IWRK3
...
! __________________________________________________________
! _________________________________________________________
      Real (kind=ksp), Dimension (:), Intent (In) :: XVALT
      Integer, Dimension (:), Intent (Out) :: IRNGT
! __________________________________________________________
!
      Integer, Dimension (:), Allocatable :: JWRKT
...
! _________________________________________________________
      ...
      Integer, Intent (InOut) :: NORD
! __________________________________________________________
      Real (kind=ksp) :: XPIV, XWRK, XWRK1, XMIN, XMAX, XPIV0
!
      Integer, Dimension (SIZE(XDONT)) :: ILOWT, IHIGT
      Integer :: NDON, JHIG, JLOW, IHIG, IWRK, IWRK1, IWRK2, IWRK3
      Integer :: IDEB, JDEB, IMIL, IFIN, NWRK, ICRS, IDCR, ILOW
...
! __________________________________________________________
      ...
      Integer :: LMTNA, LMTNC, IRNG, IRNG1, IRNG2
      Integer :: NVAL, IIND, IWRKD, IWRK, IWRKF, JINDA, IINDA, IINDB
      Real (kind=ksp) :: XTST, XVALA, XVALB
!
!
      NVAL = Min (SIZE(XVALT), SIZE(IRNGT))
      NUNI = NVAL
...
! _________________________________________________________
      Real (kind=ksp), Dimension (:), Intent (In) :: XDONT
      Real (kind=ksp) :: res_med
! __________________________________________________________
      Real (kind=ksp), Parameter :: XHUGE = HUGE (XDONT)
      Real (kind=ksp), Dimension (SIZE(XDONT)+6) :: XWRKT
      Real (kind=ksp) :: XWRK, XWRK1, XMED7
!
      Integer, Dimension ((SIZE(XDONT)+6)/7) :: ISTRT, IENDT, IMEDT
      Integer :: NDON, NTRI, NMED, NORD, NEQU, NLEQ, IMED, IDON, IDON1
      Integer :: IDEB, IWRK, IDCR, ICRS, ICRS1, ICRS2, IMED1
...
! _________________________________________________________
      Real (kind=ksp), Dimension (:), Intent (In) :: XDONT
      Real (kind=ksp) :: valnth
      Integer, Intent (In) :: NORD
! __________________________________________________________
      Real (kind=ksp), Dimension (SIZE(XDONT)) :: XLOWT, XHIGT
      Real (kind=ksp) :: XPIV, XPIV0, XWRK, XWRK1, XWRK2, XWRK3, XMIN, XMAX
!
      Integer :: NDON, JHIG, JLOW, IHIG
      Integer :: IMIL, IFIN, ICRS, IDCR, ILOW
      Integer :: JLM2, JLM1, JHM2, JHM1, INTH
