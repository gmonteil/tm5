...
contains
  ...
  pure subroutine TM5_Conv_Matrix( &
    ...
    ! dummy arrays, allocated in the parent routine to save on allocate and deallocate statements
    ! since their lower bounds are not zeros, we cannot use assumed shape arrays, but must declare the bounds explicitly
    !real, intent(inout)         ::  f(:,:), fu(:,:), amu(:), amd(:)
    real, intent(inout)         :: f(0:lmx,lmx), fu(0:lmx,lmx), amu(0:lmx), amd(0:lmx)
