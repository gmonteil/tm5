...
subroutine inverse (mat_size, A)
    ...
    call DGETRI(mat_size, A_copy, mat_size, ipvt, work, 2*mat_size, istat)
    if (istat /= 0) then
...
subroutine eigvals (mat_size, A, evals, check_positive)
    ...
    allocate(work(8*mat_size))
    allocate(failed_indices(mat_size))

    call DSYEVX('V', 'A', 'L', mat_size, A_copy, mat_size, 0.0, 0.0, 0, 0, 1.0D-15, conv_evals, evals, evecs, mat_size, work, 8*mat_size, iwork, failed_indices, istat)
    if (istat /= 0) then
...
subroutine test_eigvals(A, eval, evec)

    real, intent(in) :: eval(:)
    real, intent(in) :: A(:,:), evec(:,:)
    ...
    call matmul_fast(A, evec, eig_residues)
    do i=1,mat_size
...
subroutine orthonormalize(A)
    ...
    ! Given an MxN real matrix A, calculate an MxN real matrix Q whose
    ! columns are orthonormal and span the same space as the columns of A
    ! We will use MKL's F95 interface to avoid the huge numnber of arguments
    !
    ...
    integer                 :: M, N, dim_tau, info, status, lwork, i
    real, allocatable       :: tau(:), work(:)

    M = size(A, 1)
    N = size(A, 2)
    dim_tau = max(1, min(M, N))
    allocate(tau(dim_tau))
    ...
    if (info /= 0) then
        write(0, '("ERROR : geqrf failed with status ", i3)') info
    ...
    if (info /= 0) then
        write(0, '("ERROR : orgqr failed with status ", i3)') info
        IF_NOTOK_RETURN(status=1)
    end if

    deallocate(tau, work)

    ! MKL docs say that A is now orthogonal, not guaranteed to be orthonormal, so let's make it so
    do i = 1, N
