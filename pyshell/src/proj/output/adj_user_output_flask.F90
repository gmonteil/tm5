...
subroutine read_samples(midpt_date, i_period, status)
    ...
    select case (split_period)
        ...
        case ('y')
            write(fname,"(a,a,i4.4,a)") trim(indir_point), '/point/point_departures_', midpt_date(1), '.nc4'
        case ('m')
    ...
    if (flask_file_exists) then
        ...
        do region=1, nregions
            ...
            do itrac = 1, ntracet
                ...
                ! Guillaume Monteil, Jan 2022:
                ! Reading the "date_components" variable as a single array leads to a segmentation fault when the number of obs is large.
                ! The error is machine dependent and not related to TM5 (can be reproduced outside). It happens only when using intel compiler,
                ! and reading the data using the HDF5 library directly works without issue (so I believe it's a netCDF bug).
                ! Reading the data column by column works, but requires using the netCDF library directly (and not the file_netcdf module).
                allocate(flasks(region)%io_tracer(itrac)%times(6, nflasks))
                status = nf90_inq_varid(tgrp_id, 'date_components', varid)
                if(status /= nf90_noerr) return
                if (status == nf90_noerr) then
                    do icol = 1, 6
                        status = nf90_get_var(tgrp_id, varid, flasks(region)%io_tracer(itrac)%times(icol, :), (/icol, 1/), (/1, nflasks/))
                        IF_NOTOK_RETURN(status=1)
                    end do
                end if
                ...
                do iflask = 1,nflasks
                    ...
                    flasks(region)%io_tracer(itrac)%rif(iflask) = (flasks(region)%io_tracer(itrac)%lon(iflask)-float(xbeg(region)))/dxr + 0.9999999
                    flasks(region)%io_tracer(itrac)%rjf(iflask) = (flasks(region)%io_tracer(itrac)%lat(iflask)-float(ybeg(region)))/dyr + 0.9999999
...
subroutine adj_user_output_flask_step(region, tr, status)
    ...
    select case (split_period)
        case ('a')
            i_period = 1
        case ('y')
            i_period = midpt_date(1)-idatee(1)+1
        case ('m')
