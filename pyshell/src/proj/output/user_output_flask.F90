...
!-------------------------------------------------------------------------
  ...
  use user_output_flask_data,   only : POINT_ERROR_GRADIENT, POINT_ERROR_NEIGHBOR
...
contains
  ...
  subroutine user_output_flask_init(status)
    ...
    integer                 :: itr, error_choice_global
    character(len=256)      :: rc_key
    ...
    call ReadRc( rcF, 'output.point.errors', error_choice_global, status)
    IF_NOTOK_RETURN(status=1)
    allocate(point_error_choice(ntracet))
    do itr = 1, ntracet
        write(rc_key, '("output.point.errors.", a)') trim(names(itr))
        call ReadRc( rcF, trim(rc_key), point_error_choice(itr), status, default=error_choice_global )
        IF_ERROR_RETURN(status=1)
    end do
  ...
  subroutine user_output_flask_done(status)
    ...
    call user_output_flask_write(status)
    IF_NOTOK_RETURN(status=1)

    ! deallocate some stuff
    if (allocated(IO_tracer))           deallocate(IO_tracer)
    if (allocated(point_error_choice))  deallocate(point_error_choice)
  ...
  subroutine user_output_flask_step(region, tr, status)
    ...
    if ( .not. period_opened(i_period) ) then
        if ( .not. newsrun .and. flask_active ) then
            call user_output_flask_write(status)
            IF_NOTOK_RETURN(status=1)
        end if
  ...
  subroutine read_samples(midpt_date, i_period, status)
    ...
    select case (split_period)
        ...
        case ('y')
            write(fname,"(a,a,i4.4,a)") trim(indir_point), '/point_input_', midpt_date(1), '.nc4'
            write(outfile_point,"(a,a,i4.4,a)") trim(outdir_point), '/point/point_output_', midpt_date(1), '.nc4'
        case ('m')
    ...
    if (flask_file_exists) then
        ...
        do itrac = 1, ntracet
            ...
            do iflask=1,nflasks
                ...
                IO_tracer(itrac)%flasks(iflask)%rif = (IO_tracer(itrac)%flasks(iflask)%lon-float(xbeg(region)))/dxr + 0.9999999
                IO_tracer(itrac)%flasks(iflask)%rjf = (IO_tracer(itrac)%flasks(iflask)%lat-float(ybeg(region)))/dyr + 0.9999999
  ...
  subroutine user_output_flask_write(status)
           ...
           if(in_window)  then
              ...
              select case (vchoice)
                ...
                case (VERT_COORD_ALT)
                  ...
                  select case(l)
                  case(0) ! will never happen, because l >= 2
                ...
                case (VERT_COORD_PRES)
                  do l=0,lm(region)
                     pressure(l) = wcx       * wcy       * phlb(ifr,jfr,l+1) + &
                                   (1.0-wcx) * wcy       * phlb(ifn,jfr,l+1) + &
                                   wcx       * (1.0-wcy) * phlb(ifr,jfn,l+1) + &
                                   (1.0-wcx) * (1.0-wcy) * phlb(ifn,jfn,l+1)
                  end do

                  do l = lowest_sample_layer, lm(region) ! selects layer , note that we start from second layer from surface
                     if(pressure(l) .lt. pres) exit
                  end do

                  select case(l)
                  case(0) ! again, never going to happen
                        if (.not. IO_tracer(itrac)%flasks(iflask)%below_surface_warning) then
                            if (flask_verbose) then
                                write (*,'(a, " :: WARNING:  For flask ",i8,":")') rname, IO_tracer(itrac)%flasks(iflask)%id
                  ...
                  case default
                        lfr = l  !the site layer
                        ! the offset from the center of the layer (-0.5--->+0.5)
                        ! (interpolation is in (Pa))
                        if (pres <= pressure(l-1)) then
                            rlf = (pressure(l) - pres)/(pressure(l-1) - pressure(l)) + 0.5
                        else
                            rlf = 0.0
                        end if
                  end select
              ...
              select case (point_error_choice(itrac))
