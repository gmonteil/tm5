...
module user_output
  ...
  logical           :: profile_data   ! signal for writing profile time series at stations
...
!===========================================================================================================
  ...
  subroutine user_output_init(status)
    ...
    use user_output_profile,   only : user_output_profile_init
    ...
    if (RcHasKey(rcF, 'output.profiles', nfound)) then
        call ReadRc(rcF, 'output.profiles', profile_data, status)
        IF_NOTOK_RETURN(status=1)
    end if
    ...
    ! profile time series
    if (profile_data)   call user_output_profile_init(status)
    IF_NOTOK_RETURN(status=1)
...
!===========================================================================================================
  ...
  subroutine user_output_step ( region , tr, status )
    ...
    use user_output_profile,  only : user_output_profile_step
    ...
    IF_NOTOK_RETURN(status=1)
    if (profile_data)   call user_output_profile_step(region, tr, status)
    IF_NOTOK_RETURN(status=1)
  ...
  subroutine user_output_done ( msg, status )
    ...
    use user_output_profile,    only : user_output_profile_done
    ...
    IF_NOTOK_RETURN(status=1)
    if (profile_data)   call user_output_profile_done
