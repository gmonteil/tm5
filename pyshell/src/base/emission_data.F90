...
module Emission_Data
  ...
  type diurnal_cycle
    ...
    real, dimension(:,:,:), allocatable :: emission ! (n_tstep,n_lat,n_lon), used to hold the total emission for a timestep
    real, dimension(:,:,:), allocatable :: blh      ! (n_tstep,n_lat,n_lon), used to hold the boundary layer height
    integer, allocatable, dimension(:)  :: blh_samples ! for taking the average boundary layer height
#endif
...
!    integer                             ::  year1, year2
    ...
    do region=1,nregions
        ...
        do i_tra=1,ntracet
            ...
            !      emissions.tracer.region.categories : n_cat
            call ReadRc( rcF, 'emission.'//trim(names(i_tra))//'.'//trim(region_name(region))//'.categories', ncat, status)
...
!    call Get( time_window%t2, year=year2 )
  ...
  subroutine Cat_Info_Init( eci, line, status )
    ...
    type(T_Em_Cat_Info), intent(out)    ::  eci
    character(len=*), intent(inout)     ::  line
    integer, intent(out)                ::  status

    ! --- const -----------------------------------------------

    character(len=*), parameter         ::  rname = mname//'/Cat_Info_Init'

    ! --- local -----------------------------------------------

    integer                             ::  j
    character(len=MAX_RCVAL_LEN)        ::  tipp_string
    character(len=MAX_RCVAL_LEN)        ::  cor_string
    character(len=1),parameter          ::  cat_sep_char=';'

    ! --- begin -----------------------------------------------


    ! example line:
    !    wetlands         ; 100.0 ; 200.0-g ; 0.0-e-monthly ; 1 ; wetlands

    ! wetlands
    call goReadFromLine( line, eci%name, status, sep=cat_sep_char )
    IF_ERROR_RETURN(status=1)

    ! 100.0
    call goReadFromLine( line, eci%error, status, sep=cat_sep_char )
    IF_ERROR_RETURN(status=1)

    ! 200.0-g
    call goReadFromLine( line, cor_string, status, sep=cat_sep_char )
    IF_ERROR_RETURN(status=1)
    call goReadFromLine( cor_string, eci%corlen, status, sep='-' )
    IF_ERROR_RETURN(status=1)
    call goReadFromLine( cor_string, eci%corchoice, status, sep='-' )
    IF_ERROR_RETURN(status=1)

    ! 0.0-e-monthly
    call goReadFromLine( line, cor_string, status, sep=cat_sep_char )
    IF_ERROR_RETURN(status=1)
    call goReadFromLine( cor_string, eci%tau, status, sep='-' )
    IF_ERROR_RETURN(status=1)
    call goReadFromLine( cor_string, eci%tauchoice, status, sep='-' )
    IF_ERROR_RETURN(status=1)
    call goReadFromLine( cor_string, eci%treskey, status, sep='-' )
    IF_ERROR_RETURN(status=1)

    ! setup time profile:
    call Time_Profile_Init( eci%time_profile, time_window, trim(eci%treskey), status )
    IF_NOTOK_RETURN(status=1)

    ! number of tipp categories: 1
    call goReadFromLine( line, eci%n_tipp, status, sep=cat_sep_char )
    IF_ERROR_RETURN(status=1)

    ! storage for tipp info:
    allocate( eci%tipp(eci%n_tipp) )

    ! init tipp categories:
    do j = 1, eci%n_tipp
      ! extract tipp description:
      call goReadFromLine( line, tipp_string, status, sep=cat_sep_char )
      IF_ERROR_RETURN(status=1)
      ! fill:
      call TIPP_Info_Init( eci%tipp(j), tipp_string, status )
      IF_ERROR_RETURN(status=1)
    end do
  ...
  subroutine Cat_Info_Done( eci, status )
    ...
    ! done with tipp categories:
    do j = 1, eci%n_tipp
      ! fill:
      call TIPP_Info_Done( eci%tipp(j), status )
      IF_ERROR_RETURN(status=1)
    end do
  ...
  subroutine TIPP_Info_Done( tipp_info, status )

    ! --- in/out ----------------------------------------------

    type(T_TIPP_Info), intent(inout)    ::  tipp_info
    integer, intent(out)                ::  status

    ! --- const -----------------------------------------------

    character(len=*), parameter         ::  rname = mname//'/TIPP_Info_Done'
