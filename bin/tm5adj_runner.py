...
def load_pressurelevel_weights(mixfile : Path, lat : float, lon : float,
    ...
    for it,t in enumerate(sample_times):
        ...
        if cur_time>=timepoint:
            timedelta_lst.append( cur_time - timepoint )
        else:
            timedelta_lst.append( timepoint - cur_time )
    timedelta_lst = np.array(timedelta_lst)
    it = np.argmin(timedelta_lst)
    msg = f"time closest to pulse detected at it={it} -->{time_lst[it]}<-- (diff: {timedelta_lst[it]})"
    logger.info(msg)
    #
