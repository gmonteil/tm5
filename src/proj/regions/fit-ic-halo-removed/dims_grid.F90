...
module dims_grid
  ...
  integer, parameter  ::  len_region_name = 10 + 7 !-- for '_halorm'
  ...
  !-- 20260721: remove the HALO parts from the two inner domains
  character(len=len_region_name), parameter  ::  region_name(1:nregions) = &
       (/ 'glb600x400       ', 'eur300x200_halorm', 'gns100x100_halorm'/)
  ...
  integer, parameter  ::  xbeg    (nregions) = (/ -180, -30,  3 /)
  integer, parameter  ::  xend    (nregions) = (/  180,  48, 15 /)
  integer, parameter  ::  ybeg    (nregions) = (/  -90,  26, 44 /)
  integer, parameter  ::  yend    (nregions) = (/   90,  70, 56 /)
  integer, parameter  ::  im      (nregions) = (/   60,  26, 12 /)
  integer, parameter  ::  jm      (nregions) = (/   45,  22, 12 /)
