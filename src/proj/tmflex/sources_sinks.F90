...
module sources_sinks
  ...
  use tmflex,     only : resetBackgroundMix
...
!    print * , 'trace0: end month ', idate(2)
    ...
    do region=1,nregions
       ...
       do n=1,ntracet
          if (iniconc_from_file) then
            ...
            rm(1:im(region),1:jm(region),1:lm(region),n) = mix_ini_array(1:im(region),1:jm(region),1:lm(region)) * &
                m(1:im(region),1:jm(region),1:lm(region)) / fscale(n)
          else
  ...
  subroutine source1( region, tr, status )
    ...
    call resetBackgroundMix(region, status)
    IF_NOTOK_RETURN(status=1)
