...
contains
  ...
  subroutine readnetcdf(file_name, status)
    ...
    do region = 1, nregions
        ...
        rym = 0
        rzm = 0
        rm(xlb:xub,ylb:yub,1:lmr,1) = field4d(:,:,:,1)
       ! rm(xlb:xub,ylb:yub,1:lmr,2) = field4d(:,:,:,1)
        deallocate(field4d)

        field4d = nc_read_var(grp_id, 'rxm')
        rxm(xlb:xub,ylb:yub,1:lmr,1) = field4d(:,:,:,1)
        !rxm(xlb:xub,ylb:yub,1:lmr,2) = field4d(:,:,:,1)
        deallocate(field4d)

        field4d = nc_read_var(grp_id, 'rym')
        rym(xlb:xub,ylb:yub,1:lmr,1) = field4d(:,:,:,1)
        !rym(xlb:xub,ylb:yub,1:lmr,2) = field4d(:,:,:,1)
        deallocate(field4d)

        field4d = nc_read_var(grp_id, 'rzm')
        rzm(xlb:xub,ylb:yub,1:lmr,1) = field4d(:,:,:,1)
        !rzm(xlb:xub,ylb:yub,1:lmr,2) = field4d(:,:,:,1)
        deallocate(field4d)
...
!        ! SBf
        ...
        rxxm(xlb:xub,ylb:yub,1:lmr,1) = field4d(:,:,:,1)
        rxxm(xlb:xub,ylb:yub,1:lmr,2) = 0!field4d(:,:,:,1)
        deallocate(field4d)

        field4d = nc_read_var(grp_id, 'rxym')
        rxym(xlb:xub,ylb:yub,1:lmr,1) = field4d(:,:,:,1)
        rxym(xlb:xub,ylb:yub,1:lmr,2) = 0!field4d(:,:,:,1)
        deallocate(field4d)

        field4d = nc_read_var(grp_id, 'rxzm')
        rxzm(xlb:xub,ylb:yub,1:lmr,1) = field4d(:,:,:,1)
        rxzm(xlb:xub,ylb:yub,1:lmr,2) = 0!field4d(:,:,:,1)
        deallocate(field4d)

        field4d = nc_read_var(grp_id, 'ryym')
        ryym(xlb:xub,ylb:yub,1:lmr,1) = field4d(:,:,:,1)
        ryym(xlb:xub,ylb:yub,1:lmr,2) = 0!field4d(:,:,:,1)
        deallocate(field4d)

        field4d = nc_read_var(grp_id, 'ryzm')
        ryzm(xlb:xub,ylb:yub,1:lmr,1) = field4d(:,:,:,1)
        ryzm(xlb:xub,ylb:yub,1:lmr,2) = 0!field4d(:,:,:,1)
        deallocate(field4d)

        field4d = nc_read_var(grp_id, 'rzzm')
        rzzm(xlb:xub,ylb:yub,1:lmr,1) = field4d(:,:,:,1)
        rzzm(xlb:xub,ylb:yub,1:lmr,2) = 0!field4d(:,:,:,1)
        deallocate(field4d)
