...
!    integer             ::  year, month, day
    ...
    apply_dailycycle = tracers_em_info(region)%tracer(ico2)%dailycycle%apply ! This T/F will come from an rc file, read in by emission_fwd/emission_fwd_setup
    ...
    do i_cat = 1, tracers_em_info(region)%tracer(ico2)%n_cat
      ...
      em     => ref_emissions_apri(region)%tracer(ico2)%cat(i_cat)%field   !  kg/cl/s or factor around 0.0 or 1.0

      ! calculate the time step index for daily cycle, if it is to be applied
      apply_dailycycle = tracers_em_info(region)%tracer(ico2)%dailycycle%cycle_cat(i_cat)%apply ! this is true only if the daily cycle file read in previously has a cycle for this category
      if (apply_dailycycle) i_period_dcycle = (idate_mid(4) * 3600 + idate_mid(5) * 60 + idate_mid(6)) / &
        tracers_em_info(region)%tracer(ico2)%dailycycle%cycle_cat(i_cat)%dtime + 1 ! this is the time index within one day

      ! get period index for emissions:
      call Time_Profile_Index( tracers_em_info(region)%tracer(ico2)%cat(i_cat)%time_profile, NewDate(time6=idater), i_period, status, at_left_side=.true. )
      IF_NOTOK_RETURN(status=1)
             ...
             do i = isr(region), ier(region)
                ...
                if (apply_dailycycle) then
                    select case (dailycycle_type)
                        case (0)
                            x = emfac(i,j,1) * dtime * &
                                tracers_em_info(region)%tracer(ico2)%dailycycle%cycle_cat(i_cat)%scaling(i,j,i_period_dcycle)
                        case (1)
                            x = (emfac(i,j,1) + &
                                tracers_em_info(region)%tracer(ico2)%dailycycle%cycle_cat(i_cat)%anomaly(i,j,i_period_dcycle)) * dtime
                    end select
                else
                    x = emfac(i,j,1) * dtime   ! kg/cl/timestep
                end if ! apply_dailycycle

                rm(i,j,1,ico2)  = rm(i,j,1,ico2)  + x
#ifdef slopes
                rzm(i,j,1,ico2) = rzm(i,j,1,ico2) - x
#endif
                ! 1x1 flux
                if (write_flux1x1) then
                    do il = 1, grid_translate_1x1(region)%cell(i,j)%N
                        ...
                        flux1x1_3d%prod(i_1x1,j_1x1,1,flux1x1_tidx,ico2) = flux1x1_3d%prod(i_1x1,j_1x1,1,flux1x1_tidx,ico2) + &
                            f_1x1 * x
                    end do
                end if
                ...
                if ( apply_budget_global ) then
                   ...
                   budemig(nzone,nzone_v,ico2,i_period_budget) = budemig(nzone,nzone_v,ico2,i_period_budget) + x ! [kg]
                   !budemi_dat(region)%emi(i,j,nzone_v,ico2) = &
                   !     budemi_dat(region)%emi(i,j,nzone_v,ico2) + x ! [kg]

                   sum_emission(region,ico2) = sum_emission(region,ico2) + x  ! [kg]
                end if
#endif
             end do ! i
          end do ! j
