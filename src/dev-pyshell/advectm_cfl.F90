...
!        write (gol,'(a,"     cfl_ok : ",l1)') rname, cfl_ok; call goPr
  ...
  subroutine Setup_MassFlow( tr, ndyn, status )
    ...
    do n = 1, nregions
      !
      call mdat_TimeInterpolation( pu_dat(n), tr, status )
      IF_ERROR_RETURN(status=1)
      !
      call mdat_TimeInterpolation( pv_dat(n), tr, status )
      IF_ERROR_RETURN(status=1)
      !
    end do
 ...
 subroutine define_splitorderzoom
  ...
  subroutine dynamum( region, js,je,ls,le, cfl_ok, status )
    ...
    integer                             :: l_mloop_max
    real                                :: l_xim
    ! variables to collapse two loops into one
    integer                             :: N_tot_2d, i_tot_2d
    ...
    ! For the sake of OpenMP, create a single iterator
    N_tot_2d = (le-ls+1) * (je-js+1)
    allocate(counter(N_tot_2d, 2))
    call collapse_loop( ls, le, js, je, counter )

    !$omp parallel default ( shared ) &
    !$omp reduction(.and.:cfl_okl) reduction(max:l_mloop_max) reduction(max:l_xim) &
    !$omp private ( i_tot_2d, i, j, l, nloop, is, ie ) private ( alpha, max_alpha, mx )
    allocate(mx(0:im(region)+1)) ! allocate large enough so that it can accommodate anything
    !$omp do schedule(guided)
    do i_tot_2d = 1, N_tot_2d
        l = counter(i_tot_2d, 1)
        j = counter(i_tot_2d, 2)
  ...
  subroutine dynamvm( region, is,ie,ls,le, cfl_ok, status )
    ...
    !$omp parallel default ( shared ) &
    !$omp reduction ( .and. : cfl_okl ) reduction ( max : l_mloop_max ) reduction ( max : l_xim ) &
    !$omp private ( i, j, l ) private ( beta, max_beta, mx )
    ...
    if ( cfl_okl ) then
        ...
        ! calculate new air mass distribution
        if (region == 1) then
            mnew(1:imr, 1:jmr, ls:le) = m(1:imr, 1:jmr, ls:le) + bm(1:imr, 1:jmr, ls:le) - bm(1:imr, 2:jmr+1, ls:le)
        else if ( touch_sp(region) == 1) then
            mnew(1:imr, 1:je, ls:le) = m(1:imr, 1:je, ls:le) + bm(1:imr, 1:je, ls:le) - bm(1:imr, 2:je+1, ls:le)
        else if ( touch_np(region) == 1) then
            mnew(1:imr, js:jmr, ls:le) = m(1:imr, js:jmr, ls:le) + bm(1:imr, js:jmr, ls:le) - bm(1:imr, js+1:jmr+1, ls:le)
        else
            mnew(is:ie, js:je, ls:le) = m(is:ie, js:je, ls:le) + bm(is:ie, js:je, ls:le) - bm(is:ie, js+1:je+1, ls:le)
        end if

        ! store new air mass in m array
        if ( region == 1) then
            m(1:imr, 1:jmr, ls:le) = mnew(1:imr, 1:jmr, ls:le)
        else if ( touch_sp(region) == 1) then
            m(1:imr, 1:je, ls:le) = mnew(1:imr, 1:je, ls:le)
        else if ( touch_np(region) == 1) then
            m(1:imr, js:jmr, ls:le) = mnew(1:imr, js:jmr, ls:le)
        else
  ...
  subroutine dynamwm( region, is,ie,js,je, cfl_ok, status )
    ...
    N_tot_2d = (lmr+1) * jmr
    allocate(counter(N_tot_2d, 2))
    call collapse_loop( 0, lmr, 1, jmr, counter )

    ! compute f, pf, fx and fy
    !$omp parallel do default ( shared ) reduction (max:l_gamma) schedule(guided) &
    !$omp private ( gamma, i, j, l, i_tot_2d )
    do i_tot_2d = 1, N_tot_2d
        l = counter(i_tot_2d, 1)
        j = counter(i_tot_2d, 2)
    !do l = 0, lmr      ! 1,lmm1
       !do j = 1,jmr
          do i = 1,imr
             ...
             if ( l_gamma >= one ) exit ! exit i-loop
          end do  ! i
          ! leave after first violation:
          !if ( l_gamma >= one ) exit
       !end do  ! j
