  ...
  subroutine Diffusion_Init( status )
    ...
    if ( revert > 0 ) then
      ...
      do n = 1, nregions
        ...
        ! enable meteo:
        call mdat_set( lsmask_dat(n), status, used=.true. )
        IF_NOTOK_RETURN(status=1)

        ! enable meteo:
        call mdat_set( sr_ecm_dat(n), status, used=.true. )
        IF_NOTOK_RETURN(status=1)
        ! enable meteo:
        call mdat_set( sr_ols_dat(n), status, used=.true. )
        IF_NOTOK_RETURN(status=1)

        ! enable meteo:
        call mdat_set( u10m_dat(n), status, used=.true. )
        IF_NOTOK_RETURN(status=1)
        ! enable meteo:
        call mdat_set( v10m_dat(n), status, used=.true. )
        IF_NOTOK_RETURN(status=1)

        ! enable meteo:
        call mdat_set( slhf_dat(n), status, used=.true. )
        IF_NOTOK_RETURN(status=1)
        ! enable meteo:
        call mdat_set( sshf_dat(n), status, used=.true. )
        IF_NOTOK_RETURN(status=1)

        ! enable meteo:
        call mdat_set( ewss_dat(n), status, used=.true. )
        IF_NOTOK_RETURN(status=1)
        ! enable meteo:
        call mdat_set( nsss_dat(n), status, used=.true. )
        IF_NOTOK_RETURN(status=1)
