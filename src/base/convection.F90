  ...
  subroutine convec( region, step, tr, status )
    ...
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        !$OMP   shared ( dt ) &
        !$OMP   shared ( revert, trans ) &
        !$OMP   shared ( cloud_base, cloud_top, cloud_lfs ) &
#ifdef slopes
        !$OMP   shared ( rxm, rym, rzm ) &
#ifdef secmom
        !$omp   shared ( rxxm, ryym, rzzm, rxym, ryzm, rxzm ) &
#endif
#endif
        !$OMP   shared ( m, rm, entu, entd, detu, detd ) &
#ifndef without_diffusion
        !$OMP   shared ( dkg ) &
#endif
#ifndef without_wet_deposition
        !$OMP   shared ( cp ) &
        !$OMP   shared ( sceffdeep ) &
        !$OMP   shared ( cvsfac ) &
        !$OMP   shared ( offsetn ) &
        !$OMP   private ( zwetrm2 ) &
        !$OMP   priate ( nglob ) &
#ifdef with_budgets
        !$OMP   shared ( nzon_vg, apply_budget_global ) &
        !$OMP   shared ( buddep_dat, ra ) &
        !$OMP   reduction( + : g_sum_wet ) &
#endif
#endif
        !$OMP   private ( i, j, lmc, counter ) &
        !$OMP   private ( conv1, lbdcv ) &
        !$OMP   private ( status ) &
        !$OMP   reduction ( + : nfail )
