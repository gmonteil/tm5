#define PRINTLINE write (*,'("DEBUG :: file ", a,", module ", a, ", line ", i5)') __FILE__, rname, __LINE__
...
public :: nc_variables_deflate, nc_deflate_level, nc_var_exists

integer :: nc_status

! NF90_ENOGRP is supposed to be defined in the netcdf header. However, there is a problem with the netcdf-fortran installation on
! Zeus, and this is not defined. So I need to define it here. Please comment this out if your netcdf-fortran library defines this.
...
integer, parameter  :: NF_MAX_ATTR_LEN = 512
integer, parameter  :: NF_SHORT_STR = 80

character(len=*), parameter :: mname = 'file_netcdf'

type t_ncVar
    integer                                     :: nc_id
    integer                                     :: var_id
    character(len=NF_SHORT_STR)                 :: var_type
    character(len=NF90_MAX_NAME)                :: var_name
    character(len=NF90_MAX_NAME), allocatable   :: attr_names(:)
    character(len=NF_MAX_ATTR_LEN), allocatable :: attr_values(:)
end type t_ncVar
...
type nc_uni_att
    character(len=NF_MAX_ATTR_LEN) :: c
...
function nc_get_group(nc_id, group_name, io_status) result (group_id)
    ...
    character(len=NF90_MAX_NAME)   :: dummy_name
...
function nc_var_exists(nc_id, var_name) result (var_exists)
    ! returns a boolean depending on whether a variable exists or not
...
function nc_grp_exists(nc_id, group_name, io_status) result (grp_exists)
    ...
    if (nc_status == NF90_NOERR) then
        grp_exists = .true.
    else if (nc_status == NF90_ENOGRP) then
        grp_exists = .false.
    else
        if (.not. present(io_status)) then
            dummy_int = nf90_inq_grpname_full(nc_id, dummy_len, dummy_name)
...
function nc_open(filename, mode, status) result (output_ncid)

    use os_specs, only : MAX_FILENAME_LEN, WRITE_STR_LEN
    implicit none
    ...
    integer                        :: output_ncid
    logical                        :: path_exist
    character(len=WRITE_STR_LEN)   :: err_msg
    character(len=MAX_FILENAME_LEN):: dir_name
    integer                        :: sep_pos

    character(len=*), parameter    :: rname = mname//'/nc_open'

    ! check first if file exists or not, in case mode is 'r' or 'a'
    ! if mode is 'w' or 'c' create the folder if it does not exist
    select case (mode)
        case ('r', 'a')
            inquire(file=filename, exist=path_exist)
            if (.not. path_exist) then
                write(gol, '("File ", a, " does not exist")') trim(filename) ; call goErr
                status = 1
                IF_NOTOK_RETURN(status=1)
            end if

        case ('w', 'c')
            sep_pos = index(filename, '/', .true.)
            if (sep_pos .gt. 1) then
                dir_name = filename(1:sep_pos-1)
                inquire(file=trim(dir_name)//'/.', exist=path_exist)
                if (.not. path_exist) call system('mkdir -p '//trim(dir_name))
            end if
...
function nc_create_var(nc_id, var_name, dim_list, var_type, io_status) result (output_varid)
    ...
    do i = 1, size(dim_list)
        ...
        do while (nf90_inq_dimid(group_id, trim(adjustl(dim_list(i))), j) == NF90_EBADDIM)
            ...
            if (nc_status == NF90_ENOGRP) call nc_error('Root group reached, dimension '//trim(adjustl(dim_list(i)))//' still not found')
            group_id = parent_id
        end do
        dim_id_list(i) = j
    end do
...
subroutine nc_set_attrs_var(nc_var, attr_names, attr_values, io_status)
    ...
    integer, optional, intent(out) :: io_status
    character(len=NF90_MAX_NAME)   :: fq_varname, fq_grpname
...
function nc_get_attr_varid(nc_id, var_id, attr_name, io_status) result (out_attr)
    ...
    integer, optional, intent(out) :: io_status
    type(nc_uni_att)               :: out_attr
    character(len=NF90_MAX_NAME)   :: fq_grpname, fq_varname
...
subroutine nc_var_to_array_i1_1d(out_array, in_nc_var)
    ...
    if (allocated(in_nc_var%i1_1d)) then
        ...
        call nc_error( 'source array not allocated; assignment to wrong type/kind?' )
    end if
